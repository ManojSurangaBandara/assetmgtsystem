<?php
			...
			$row++;
			$header = array('Identification No.', 'Category', 'Description', 'Make', 'Model','Asset No', 'Engine No', 'Army No', ($disposal == 1 ? 'Disposed Dte' : 'Ownership'), 'DOP', 'DOR', 'Unit Value');        
			...
			$exp['engineno'],
            $exp['armyno'],
            ($disposal == 1 ? $row['disposedDate'] : $row['ownerShip']),
			$exp['purchasedDate'],
			$exp['receivedDate'],
            $exp['unitValue']),$row);
            $row++;
			...
			$col = $col + 4;
			$xls->writeCell($boardMemberNumber1,$row,$col);
            $xls->download("Vehicle Details.xls");
?>      
...
$(".savebttn").click(function(){
	var id = $(this).attr('id');
	var confirm = ($('#confirm_'+id).prop('checked')) ? 1 : 0;
	if (confirm == 1) {
		$("#"+id).prop('value', 'Approved');
	} else {
		$("#"+id).prop('value', 'Approve');	
	}
	var querystring = {
			id: id,
			apprived: confirm, 				 
			action: 'bulk_approved_save'
		}
                                            ...
                                            <tbody>
                                                ...
                                                <?php foreach ($items as $exp) { ?>																
                                                    <tr>
																...
																<input type="checkbox" name="confirm_<?php echo $exp['id']; ?>" id="confirm_<?php echo $exp['id']; ?>"/> 
																<input class = "savebttn" id = "<?php echo $exp['id']; ?>" name="submit" type="submit" value="Approve"/></form></nobr></td>

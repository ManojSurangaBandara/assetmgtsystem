	...
	onClick: function(node){
        ...
        function processResponse(result) {
			...
			$.each(land, function(index, item){
				sno = 1 + index; 
				total = total + parseFloat(item.constructionCost, 2);
				parch1 = parseFloat(item.parch);
				var construction = parseFloat(item.constructionCost);
				table.append("<tr><td><nobr>"+ sno +"</td><td><nobr>"+item.assetunit+"</td><td><nobr>"+item.identificationno+"</td><td><nobr>"+item.buildingCategory+"</td><td><nobr>"+item.buildingType+"</td><td><nobr>"+item.landname+"</td><td><nobr>"+item.assetsno+"</td><td><nobr>"+item.classificationno+"</td><td><nobr>"+item.district+"</td><td><nobr>"+item.dsDivision+"</td><td><nobr>"+item.gsDivision+"</td><td><nobr>"+item.acquisitiondate+"</td><td align='right'><nobr>"+ formatCurrency(item.area) +"</td><td align='right'><nobr>"+ formatCurrency(item.feets) +"</td><td align='right'><nobr>"+ formatCurrency(construction) +"</td></tr>");
				});
        		table.append("<tr><td></td><td>Total Value</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td align='right'>"+ formatCurrency(total) +"</td></tr>");
			} // end processData
...
<div class="inner">
    <div class="section table_section">
        <div class="title_wrapper">
            <h2><?php echo $subMenu[0][$lang]?> - Non Public Fund</h2>

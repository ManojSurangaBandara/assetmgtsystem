...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $header = array('SNo', 'Identification No.', 'Building Category', 'Building Type', 'District', 'DS Division', 'GS Division', 'Floors', 'Area (sqrt)', 'Construction Cost', 'Valuation Cost', 'Con.Year');
        // Line break
        $this->Ln(10);
        ...
        // Header
       $w = array(10, 55, 60, 40, 40, 40, 40, 15, 20, 30, 30, 20);
...
// Page footer
    ...
    function FancyTable($header, $data) {
        ...
        $this->SetFont('', 'B');
        // Header
      $w = array(10, 55, 60, 40, 40, 40, 40, 15, 20, 30, 30, 20, 50);
        //  for ($i = 0; $i < count($header); $i++)
        //       $this->Cell($w[$i], 7, $header[$i], 1, 0, 'C', true);
        // $this->Ln();
 ...
 foreach ($data as $row) {
                        ...
                        $this->Cell($w[2], 6, substr($row['buildingCategory'],0,25), 'LR', 0, 'L', $fill);
                        $this->Cell($w[3], 6, substr($row['buildingType'],0,18), 'LR', 0, 'L', $fill);
                        $this->Cell($w[4], 6, $row['district'], 'LR', 0, 'L', $fill);
                        ...
                        $this->Cell($w[10], 6,number_format((float)$row['alterationValue'], 2), 'LR', 0, 'R', $fill);
...
// $pdf = new PDF();
// Column headings
$header = array('SNo', 'Identification No.', 'Building Category', 'Building Type', 'Name of Land', 'District', 'DS Division', 'Area (sqrt)', 'Construction Cost', 'No. of Floors', 'DOR');
...
$pdf->Output('Building Details Details ' . '.pdf', 'D');

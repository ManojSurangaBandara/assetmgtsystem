...
/*         $('tr').dblclick(function () {
            ...
            var querystring = {
                id: id,
                action: 'get_ordinance_places_detail'
            }
                        ...
                        $("#confirm").dialog({
                            ...
                            buttons: {
                                "Confirm": function () {
                                    var querystring = {
                                        id: id,
                                        action: 'delete_ordinance_places_details'
                                    }
                                    $.get('index.php', querystring, processResponse);
...
/*         $("#code").blur(function () {
            if ($("#code").val() != "") {
                var querystring = {
                    id: $("#code").val(),
                    action: 'get_ordinance_places_detail_code'
                }
                $.get('index.php', querystring, processResponse);
                function processResponse(result) {
        ...
        {
            var querystring = {
                action: 'get_ordinance_places_table'
            }
            $.get('index.php', querystring, processResponse);
            function processResponse(result) {
                var data = $.parseJSON(result);
                ...
                $.each(data, function (key, value) {
                    html = '<tr id="' + value.id + '"><td>' + i +
							...
							'</td><td>' + value.name_tamil +
                            '</td><td>' + value.address + '</td></tr>';
                            ...
                            <div class="sct_left">
                                <div class="sct_right">
                                    ...
                                    <form name="add_form" method="post" id="add_form" action="index.php" class="add_form"> 
                                        ...
                                        <div><label for="name" class="label">Address:</label><input type="text" class="text" name="address"  id="address" style="width:300px"/></div>

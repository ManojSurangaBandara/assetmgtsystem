...
/***************************************************************************************************/
	...
	{
        var querystring = {
            action: 'getItemtype_dos'
        }
        $.get('index.php', querystring, processResponse);
        function processResponse(result) {
			var itemtype = $.parseJSON(result);
            var option = '<option value="">---</option>';
            $.each(itemtype, function(key, value) {
				option += '<option value="' + value.itemtype + '">' + value.itemtype + '</option>';
            });
...
/***************************************************************************************************/
	...
	{
        var querystring = {
            action: 'getQstore_dos'
        }
        $.get('index.php', querystring, processResponse);
        function processResponse(result) {
			var qstore = $.parseJSON(result);
            var option = '<option value="">---</option>';
            $.each(qstore, function(key, value) {
				option += '<option value="' + value.qstore + '">' + value.qstore + '</option>';
            });
            $('#qstore').html(option);
        } // end processData
    };
/***************************************************************************************************/
    function getVotehead()
	{
        var querystring = {
            action: 'getVotehead_dos'
        }
        $.get('index.php', querystring, processResponse);
        function processResponse(result) {
			var votehead = $.parseJSON(result);
            var option = '<option value="">---</option>';
            $.each(votehead, function(key, value) {
				option += '<option value="' + value.votehead + '">' + value.votehead + '</option>';
            });
            $('#votehead').html(option);
        } // end processData
    };
/***************************************************************************************************/
	...
	{
        var querystring = {
            action: 'getVotename_dos'
        }
        $.get('index.php', querystring, processResponse);
        function processResponse(result) {
			var votename = $.parseJSON(result);
            var option = '<option value="">---</option>';
            $.each(votename, function(key, value) {
				option += '<option value="' + value.votename + '">' + value.votename + '</option>';
            });
            $('#votename').html(option);
        } // end processData
    };
/***************************************************************************************************/	
	$("#abc").on('dblclick','tr',function(e) {
	   $("html, body").animate({scrollTop: 0}, "slow");
	  var itemtype = $(this).closest('tr').find('td:eq(1)').text();
	  var itemcode = $(this).closest('tr').find('td:eq(3)').text();
	  var description = $(this).closest('tr').find('td:eq(4)').text();
	  var AsstNo = $(this).closest('tr').find('td:eq(5)').text();
	  var qstore = $(this).closest('tr').find('td:eq(6)').text();
	  var votehead = $(this).closest('tr').find('td:eq(7)').text();
	  var votename = $(this).closest('tr').find('td:eq(8)').text();
	 $('#itemtype').val(itemtype);
	 $('#itemcode').val(itemcode);
	 $('#description').val(description);
	 $('#AsstNo').val(AsstNo);
	 $('#qstore').val(qstore);
	 $('#votehead').val(votehead);
...
$( "#deleteBtn" ).click(function() {
	if ($('#itemcode').val() == ""){
			BootstrapDialog.show({
                ...
                message: 'First Select the Item to Delete'
            });   
	} else {
	...
	if(result) {					
		var querystring = {
						...
						action: 'deleteDetailsByitemcode'
						}
		$.get('index.php', querystring, processResponse);
